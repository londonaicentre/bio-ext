    ...
    try:
        for field in fieldlist:
            query = {
                "size": 0,
                "aggs": {
                    "top_keywords": {"terms": {"field": field, "size": 10}},
                    "unique_counts": {"cardinality": {"field": field}},
                },
            }
        ...
        result[field] = {"top_10": None, "unique_count": None}
...
def get_date_ranges(_es, indexname, fieldlist):
    ...
    try:
        for field in fieldlist:
            ...
            min_utc = datetime.utcfromtimestamp(min_date / 1000).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
            max_utc = datetime.utcfromtimestamp(max_date / 1000).strftime(
                "%Y-%m-%d %H:%M:%S"
            )

            result[field] = {"min_date": min_utc, "max_date": max_utc}
    except KeyError:
        print(
            f"Warning: datefield{field} not found in index {indexname} or is wrong dtype"
        )
        result[field] = {"min_date": None, "max_date": None}
    except Exception as e:
        ...
        for field in fieldlist:
            query = {
                ...
                "aggs": {
                    ...
                    "percentiles": {
                        "percentiles": {"field": field, "percents": [25, 50, 75]}
                    },
                },
            }
