...
@pytest.mark.skip(
    reason="unable to test XNAT features on github actions currently. Not been able to create a connection to the test XNAT instance"
)
class TestLoadImageXNATd:
    ...
    def test_create_dataloader_with_transform(self):
            ...
            for exp in subject_data.experiments:
                if "MR_TEST_EXPERIMENT" in subject_data.experiments[exp].label:
                    for scan in subject_data.experiments[exp].scans:
                        if (
                            "SlicePosition"
                            in subject_data.experiments[exp]
                            .scans[scan]
                            .series_description
                        ):
                            return (
                                subject_data.experiments[exp].scans[scan],
                                "test_image",
                            )
        ...
        self.dataset = CacheDataset(
            data=self.test_data, transform=self.train_transforms
        )
