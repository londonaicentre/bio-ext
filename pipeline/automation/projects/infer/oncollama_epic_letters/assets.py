...
@asset(
    ...
    deps=["epic_oncology_letters"],
    partitions_def=epic_daily_partitions,
    required_resource_keys={"bioext_elastic", "slack"},
    group_name="infer",
)
def oncollama_epic_letter(context: AssetExecutionContext, config: OncollamaConfig):
    ...
    create_index_if_not_exists(dest_es, config.target_index)
